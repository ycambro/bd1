...
public class CityFrame extends JDialog implements ActionListener {
    ...
    public CityFrame(MainFrame father, boolean modal) {
    ...
    public void getReport(String city) {
        try {

            var now = new Date();
            var formats = new SimpleDateFormat("dd-MM-YY");

            var report = storage.find((formats.format(now)) + "-" + ReportType.BY_CITY + "-" + city);

            if (report == null) {
                var weatherService = APP_CONTEXT.getWeatherService();
                var newReport = weatherService.getByCity(city);
